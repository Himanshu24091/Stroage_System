...
def admin_login_page():
    ...
    # If already logged in as Super Admin AND active tab session cookie exists, go to Admin Dashboard
    if session.get("auth_type") == "admin" and session.get("is_admin") and "vault_tab_session" in request.cookies:
        return redirect(url_for("view_bp.admin_page"))

    # If logged in as standard user with active tab session, block them with 403 Access Denied
    if session.get("user_id") and "vault_tab_session" in request.cookies:
